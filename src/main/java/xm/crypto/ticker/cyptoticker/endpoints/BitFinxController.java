...
@RestController
public class BitFinxController {
	
	@Autowired
	private DatabaseController databaseController;
	
	@GetMapping("/tickerBitfinex")
	public ResponseEntity<Map<String, Object>> getTicker() throws IOException, ClassNotFoundException, SQLException {
		OkHttpClient client = new OkHttpClient();
		Request request = new Request.Builder().url("https://api.bitfinex.com/v1/pubticker/btcusd")
				.method("GET", null).build();
		Response response = client.newCall(request).execute();
		String responseData = response.body().string();
		JSONObject jsonObject = new JSONObject(responseData);
		String timestamp = jsonObject.getString("timestamp");
		double lastPrice = jsonObject.getDouble("last_price");
